...
resource "compute",
  :cookbook => "core.1.compute",
  :design => true,
  :requires => { "constraint" => "1..1", "services" => "compute,dns,*mirror,*resource-name,*windows-domain" },
  :attributes => { "size"    => "S"
                 },
  :monitors => {
  ...
  :payloads => {
    'os' => {
      ...
      'definition' => '{
         ...
         "direction": "to",
         "targetClassName": "manifest.core.1.Compute",
         "relations": [
           { "returnObject": true,
             "returnRelation": false,
             "relationName": "manifest.DependsOn",
             "direction": "to",
             "targetClassName": "manifest.core.1.Os"
           }
         ]
      }'
    }
...
'environment' => {
    ...
    'definition' => '{
       ...
       "relationName": "base.RealizedAs",
       "direction": "to",
       "targetClassName": "manifest.core.1.Fqdn",
       "relations": [
         { "returnObject": false,
           "returnRelation": false,
           "relationName": "manifest.Requires",
             ...
             { "returnObject": false,
               ...
               "relations": [
                 { "returnObject": true,
                   ...
                   "direction": "from",
                   "targetClassName": "cloud.service.core.1.Netscaler"
                 },
                 { "returnObject": true,
                   "returnRelation": false,
                   "relationName": "base.Provides",
                 ...
                 { "returnObject": true,
                   ...
                   "relationAttrs":[{"attributeName":"service", "condition":"eq", "avalue":"gdns"}],
                   "direction": "from",
                   "targetClassName": "cloud.service.core.1.Route53"
                 },
                 { "returnObject": true,
                   "returnRelation": false,
                   "relationName": "base.Provides",
                   "relationAttrs":[{"attributeName":"service", "condition":"eq", "avalue":"gdns"}],
                   "direction": "from",
                   "targetClassName": "cloud.service.core.1.Designate"
                 },
                 { "returnObject": true,
                   "returnRelation": false,
                   "relationName": "base.Provides",
                   "relationAttrs":[{"attributeName":"service", "condition":"eq", "avalue":"gdns"}],
                   "direction": "from",
                   "targetClassName": "cloud.service.core.1.Rackspacedns"
                 },
                 { "returnObject": true,
                   "returnRelation": false,
                   "relationAttrs":[{"attributeName":"service", "condition":"eq", "avalue":"gdns"}],
                   "relationName": "base.Provides",
                   "direction": "from",
                   "targetClassName": "cloud.service.core.1.Azuretrafficmanager"
                 }
               ]
             }
           ]
...
'organization' => {
 ...
 'lb' => {
    ...
    'definition' => '{
       ...
       "direction": "from",
       "targetClassName": "bom.core.1.Lb",
       "relations": [
         { "returnObject": false,
           "returnRelation": false,
           "relationName": "base.RealizedAs",
           "direction": "to",
           "targetClassName": "manifest.core.1.Lb",
           "relations": [
             { "returnObject": true,
               "returnRelation": false,
               "relationName": "base.RealizedAs",
                 ...
                 { "returnObject": false,
                   ...
                   { "returnObject": true,
                     ...
                     { "returnObject": true,
                       ...
                       "relationAttrs":[{"attributeName":"service", "condition":"eq", "avalue":"dns"}],
                       "direction": "from",
                       "targetClassName": "cloud.service.core.1.Infoblox"
                     }
                   ]
                 }
               ]
...
resource "storage",
  :cookbook => "core.1.storage",
  :design => true,
  :attributes => {
    "size"        => '20G',
    "slice_count" => '1'
  },
  :requires => { "constraint" => "0..*", "services" => "storage,compute" },
  :payloads => {
    'volumes' => {
     'description' => 'volumes',
     'definition' => '{
       ...
       "relationName": "base.RealizedAs",
       "direction": "to",
       "targetClassName": "manifest.core.1.Storage",
       "relations": [
         { "returnObject": true,
           "returnRelation": false,
           "relationName": "manifest.DependsOn",
           "direction": "to",
           "targetClassName": "manifest.core.1.Volume"
         }
       ]
     }'
   }
...
resource 'iscsi-storage',
  :cookbook => "core.1.iscsi-storage",
  :design => true,
  :attributes => {},
  :requires => { "constraint" => "0..1", "services" => "iscsi" },
  :payloads => {
    'volumes' => {
     'description' => 'volumes',
     'definition' => '{
       ...
       "relationName": "base.RealizedAs",
       "direction": "to",
       "targetClassName": "manifest.core.1.Iscsi-storage",
       "relations": [
         { "returnObject": true,
           "returnRelation": false,
           "relationName": "manifest.DependsOn",
...
resource "library",
  :cookbook => "core.1.library",
  :design => true,
  :requires => { "constraint" => "0..*" }

resource "file",
  :cookbook => "core.1.file",
  :design => true,
  :requires => {
    :constraint => "0..*",
    :help => <<-eos
...
resource "download",
  :cookbook => "core.1.download",
  :design => true,
  :requires => { "constraint" => "0..*" }

resource "daemon",
  :cookbook => "core.1.daemon",
  :design => true,
  :requires => { "constraint" => "0..*" },
  :monitors => {
...
resource "sensuclient",
   :cookbook => "core.1.sensuclient",
   :design => true,
   :requires => {"constraint" => "0..1"}

resource "firewall",
 :cookbook => "core.1.firewall",
 :design => true,
 :requires => {
   "constraint" => "0..1",
   'services' => 'firewall'
 ...
 :payloads => {
   'computes' => {
     ...
     'definition' => '{
       ...
       "relationName": "base.RealizedAs",
       "direction": "to",
       "targetClassName": "manifest.core.1.Firewall",
       "relations": [
         { "returnObject": false,
           "returnRelation": false,
           "relationName": "manifest.DependsOn",
           ...
           "relations": [
             ...
             "relationName": "base.RealizedAs",
             "direction": "from",
             "targetClassName": "bom.core.1.Compute"
             }
           ]
         }
       ]
...
resource "artifact",
  :cookbook => "core.1.artifact",
  :design => true,
  :requires => { "constraint" => "0..*" }


resource "snapshot",
  :cookbook => "core.1.snapshot",
  :design => false,
  :attributes => {
  },
  :requires => {
  ...
  :payloads => { 'cloneinstanceof' => {
    'description' => 'cloneinstanceof',
    'definition' => '{
    "returnObject": true,
    "returnRelation": false,
    "relationName": "bom.CloneInstanceOf",
    "direction": "to"
    }'
  },
  'region' => {
...
# clone_instance_of
[ { :from => 'compute',     :to => 'snapshot' } ].each do |link|
  relation "#{link[:from]}::clone_instance_of::#{link[:to]}",
  :except => [ '_default' ],
  :relation_name => 'CloneInstanceOf',
...
# propagation rule for replace and updating /etc/profile.d/core.sh
...
[ 'os', 'telegraf', 'user', 'job', 'file', 'volume', 'share', 'download', 'library', 'daemon',
  'certificate', 'logstash', 'sensuclient', 'artifact', 'objectstore', 'iscsi-storage'].each do |from|
