...
1. View Only Admin Console:
  ...
  By default, ActiveMQ Server Pack provides a view only version of Apache ActiveMQ admin console. It does not support operations such as creating queues or topics, deleting or purging messages from queues, or sending messages to queues or topics. These operations through admin console bypasses Prana deployment process, may result in ActiveMQ server state to be inconsistent to the record of truth in Prana, thus is highly discouraged.

    If one creates a queue or topic via admin console, the created queue or topic is not in Prana repository for the ActiveMQ Server Pack server instance, making the state of the server to be out of synch with the record of truth in Prana. Further, the created queue or topic via the admin console will not survive a redeployment of ActiveMQ server, i.e. a redeployment of a ActiveMQ server instance will reset the server state to what is recorded in Prana.

    Deleting or purging messages from a destination via admin console will not leave any audit trail in Prana log, so this functionality is disabled in the admin console by default.
...
ActiveMQ server from this pack supports nio or tcp transport connector, and the default port is 61616. Users can add more transport connectors with port specifications, or modify the default nio or tcp transport connection port from 61616 to another port via ActiveMQ Server Pack design GUI in Prana.
...
For ActiveMQ server provisioning, one needs to create an Prana assembly first so that an Prana platform for ActiveMQ can be added to the assembly. Users are assumed to have operating knowledge of Prana. For help with open source product Prana, please refer to [Prana Getting Started](http://prana.github.io/user/getting-started/).

To add a ActiveMQ server platform to your assembly, in Prana GUI, follow the following sample steps:
...
After ActiveMQ platform is created, all ActiveMQ components will be available in the Prana design page for the platform instance. Descriptions of ActiveMQ components are listed below:

**compute**

Your ActiveMQ instance(s) will be deployed on Prana provisioned compute(s), so the first thing to customize in your platform design is the compute component.
...
It is strongly encouraged to specify the data directory to be at a different location from ActiveMQ installation directory as configuration changes in ActiveMQ could cause a re-build of the server instance by Prana ActiveMQ cookbooks.
...
<a name="queuefields"></a>Following is a list of the **Queue GUI fields:**
  ...
  When "CompositeQueue" is selected as the destination type, there will be a Composite Queue Definition text area added to the Prana design GUI to define the composite queue.
  ...
  An optional field "Composite Queue Defination" may appear in Prana GUI if the destination type is selected as CompositeQueue. This is a free entry text area for use to define the composite queue.
...
<a name="topicfields"></a>Following is a list of the **Topic GUI fields:**
  ...
  When the destination type is CompositeTopic or VirtualTopic, an extra free entry text area will be rendered on the Prana design GUI for use to define the CompositeTopic or VirtualTopic.
  ...
  When destination type is selected as CompositeTopic or VirtualTopic, an extra free entry text area will be rendered on Prana design GUI to user to enter the CompositeTopic or VirtualTopic Definition.
...
Note: Fields with a star symbal is required, e.g. version*.
  ...
  14. Adhoc Operations Support
    ...
    Flag for enabling or disabling admin console operations such as creating, deleting, or purging messages from destination. It is recommended to keep this feature disabled as those operations should be through Prana; Enabling it will result in Prana deployment out of synch with ActiveMQ.

  15. Admin REST API Support

    Flag for enabling or disabling Admin REST API. Enabling Admin REST API allows operations such as destination creation or deletion via Jolokia REST call. It is recommended to keep this feature disabled as actions via REST API will bypass Prana, resulting in Prana deployment out of synch with ActiveMQ.
  ...
  18. Plugins Support
    ...
    Paste the following text in the free entry text area for Plugin Support field in Prana GUI:
...
On your Prana assembly design page, click on your ActiveMQ platform "myActivMQ", then "edit".
...
The ActiveMQ server will be deployed to a Prana created VM on defaut tcp port 61616.

####Queue and Topic

Let us create a queue and a topic.

In Prana design page for "myActiveMQ" platform, click the "+" button next to the "queue" component to add a new queue.
...
In Prana transition, we add a new environment, and select compute from the cloud.
...
* specify key store path
  ...
  "Keystore absolute path", default to $AX_LOCAL{keystorepath}
...
Prana GUI provide built in action panels that ActiveMQ pack implement for server start, stop, restart, repair operations.

Users are encouraged to manage ActiveMQ server life cycle via Prana operations page.
